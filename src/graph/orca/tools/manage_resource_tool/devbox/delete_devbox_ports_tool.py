"""
Delete devbox ports tool for the manage resource agent.
Handles devbox port deletion operations with state management.
"""
...
@tool
async def delete_devbox_ports_tool(
    devbox_name: str,
    ports: List[int],
    state: Annotated[dict, InjectedState],
) -> Dict[str, Any]:
    """
    Delete ports from a devbox instance.
    ...
    Args:
        devbox_name: Name of the devbox to delete ports from
...
if __name__ == "__main__":
    # Test the delete devbox ports tool
    # Run with: python -m src.graph.orca.tools.manage_resource_tool.devbox.delete_devbox_ports_tool
    ...
    print("Testing delete_devbox_ports_tool...")
    try:
        ...
        result = delete_devbox_ports_tool.invoke(
            {"devbox_name": "test-devbox", "ports": [8080, 3000], "state": mock_state}
        )
        print("✅ Delete devbox ports tool test successful!")
        print(f"Result: {result}")
    except Exception as e:
        print(f"❌ Delete devbox ports tool test failed: {e}")

    print(f"Tool name: {delete_devbox_ports_tool.name}")
    print(f"Tool description: {delete_devbox_ports_tool.description}")
